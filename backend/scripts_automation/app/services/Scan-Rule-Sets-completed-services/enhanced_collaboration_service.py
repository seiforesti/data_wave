...
from app.models.Scan-Rule-Sets-completed-models.enhanced_collaboration_models import (
    ...
    Discussion, TeamMemberRole, ReviewStatus, CommentType, KnowledgeType,
    DiscussionType, VisibilityLevel
)
...
class EnhancedCollaborationService:
    ...
    async def get_team_hub(
        ...
        try:
            ...
            if include_members:
                members_statement = select(TeamMember).where(
    ...
    async def add_team_member(
        ...
        try:
            ...
            existing_statement = select(TeamMember).where(
    ...
    async def get_rule_review(
        ...
        try:
            statement = select(RuleReview).where(RuleReview.id == review_id)
            result = session.exec(statement)
            review = result.first()
            ...
            if include_comments:
                comments_statement = select(Comment).where(
                    and_(Comment.entity_type == "review", Comment.entity_id == review_id)
    ...
    async def get_entity_comments(
        ...
        try:
            ...
            if not include_replies:
                conditions.append(Comment.parent_comment_id.is_(None))
            
            statement = select(Comment).where(and_(*conditions))
            
            # Apply sorting
            if sort_order == "oldest":
                statement = statement.order_by(Comment.created_at)
            elif sort_order == "likes":
                statement = statement.order_by(Comment.likes_count.desc())
            else:  # newest
    ...
    async def list_discussions(
        ...
        try:
            ...
            if category:
                conditions.append(Discussion.category == category)
            
            if discussion_type:
                conditions.append(Discussion.discussion_type == discussion_type)
            
            if status == "active":
                conditions.append(Discussion.is_locked == False)
            ...
            # Apply sorting
            if sort_by == "popular":
                statement = statement.order_by(Discussion.views_count.desc())
            elif sort_by == "replies":
                statement = statement.order_by(Discussion.replies_count.desc())
            else:  # recent
                statement = statement.order_by(Discussion.updated_at.desc())
    ...
    async def _organize_comments_hierarchy(self, comments: List[Comment]) -> List[Comment]:
