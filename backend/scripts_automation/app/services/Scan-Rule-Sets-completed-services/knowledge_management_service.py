...
from app.models.Scan-Rule-Sets-completed-models.enhanced_collaboration_models import (
    KnowledgeBase, ExpertConsultation,
...
from app.models.Scan-Rule-Sets-completed-models.enhanced_collaboration_models import RuleReview
...
class KnowledgeManagementService:
    ...
    async def _create_knowledge_item_internal(
        ...
        try:
            ...
            enhanced_content = await self._enhance_knowledge_content(
                ...
                knowledge_request.knowledge_type
            )
            ...
            auto_tags = await self._auto_generate_tags(enhanced_content, knowledge_request.knowledge_type)
            ...
            knowledge_item = KnowledgeBase(
                ...
                related_rule_ids=knowledge_request.related_rule_ids,
                prerequisite_knowledge=knowledge_request.prerequisite_knowledge,
                learning_objectives=knowledge_request.learning_objectives,
                content_embeddings=embeddings,
                auto_generated_tags=auto_tags,
                tags=knowledge_request.tags + auto_tags,
                metadata=knowledge_request.metadata,
                is_public=knowledge_request.is_public,
                requires_approval=knowledge_request.requires_approval
            )

            # Set approval status based on user permissions and content type
            if knowledge_request.requires_approval:
                knowledge_item.approval_status = "pending"
                await self._submit_for_approval(knowledge_item, db)
            else:
            ...
            return KnowledgeBaseResponse(
                ...
                difficulty_level=knowledge_item.difficulty_level,
                approval_status=knowledge_item.approval_status,
                view_count=knowledge_item.view_count,
                useful_votes=knowledge_item.useful_votes,
    ...
    async def _search_knowledge_internal(
        ...
        try:
            ...
            # Apply filters
            if knowledge_type:
                base_query = base_query.where(KnowledgeBase.knowledge_type == knowledge_type)
            if category:
                base_query = base_query.where(KnowledgeBase.category == category)
            if difficulty_level:
                base_query = base_query.where(KnowledgeBase.difficulty_level == difficulty_level)
            ...
            if semantic_search and self.semantic_search_index:
                ...
                # Text-based search
                text_filter = or_(
                    KnowledgeBase.title.icontains(query),
                    KnowledgeBase.content.icontains(query),
                    KnowledgeBase.tags.any(query)
                )
                ...
                base_query = base_query.order_by(
                    ...
                    desc(KnowledgeBase.useful_votes),
    ...
    async def _request_expert_consultation_internal(
        ...
        try:
            ...
            return ExpertConsultationResponse(
                ...
                estimated_duration=consultation.estimated_duration,
            ...
            if time_period:
                ...
                base_query = base_query.where(
                    and_(
                        ...
                        KnowledgeBase.created_at <= end_date
                    )
                )
    ...
    async def _enhance_knowledge_content(
        ...
        try:
            ...
            elif knowledge_type == KnowledgeType.TUTORIAL:
