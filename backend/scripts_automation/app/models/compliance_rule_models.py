...
compliance_rule_data_source_link = Table(
    ...
    SQLModel.metadata,
    Column("compliance_rule_id", Field(foreign_key="compliance_rules.id", primary_key=True)),
    Column("data_source_id", Field(foreign_key="datasource.id", primary_key=True)),
)
...
class ComplianceRule(SQLModel, table=True):
    ...
    custom_scan_rule_ids: List[int] = Field(default_factory=list, sa_column_kwargs={"type_": "JSON"})
    ...
    scan_integration_config: Dict[str, Any] = Field(default_factory=dict, sa_column_kwargs={"type_": "JSON"})
    auto_scan_on_evaluation: bool = Field(default=False)
    scan_triggers: List[str] = Field(default_factory=list, sa_column_kwargs={"type_": "JSON"})  # schedule, on_change, manual
    ...
    data_source_filters: Dict[str, Any] = Field(default_factory=dict, sa_column_kwargs={"type_": "JSON"})
    applies_to_all_sources: bool = Field(default=False)
    source_type_filters: List[str] = Field(default_factory=list, sa_column_kwargs={"type_": "JSON"})
    ...
    performance_thresholds: Dict[str, Any] = Field(default_factory=dict, sa_column_kwargs={"type_": "JSON"})
    alert_conditions: Dict[str, Any] = Field(default_factory=dict, sa_column_kwargs={"type_": "JSON"})
...
class ComplianceRuleEvaluation(SQLModel, table=True):
    ...
    entity_count: Dict[str, int] = Field(sa_column_kwargs={"type_": "JSON"})
    ...
    scan_results: Optional[Dict[str, Any]] = Field(default=None, sa_column_kwargs={"type_": "JSON"})
    performance_metrics: Optional[Dict[str, Any]] = Field(default=None, sa_column_kwargs={"type_": "JSON"})
    security_checks: Optional[Dict[str, Any]] = Field(default=None, sa_column_kwargs={"type_": "JSON"})
...
class ComplianceIssue(SQLModel, table=True):
    ...
    remediation_steps: List[str] = Field(default_factory=list, sa_column_kwargs={"type_": "JSON"})
    ...
    related_issues: List[int] = Field(default_factory=list, sa_column_kwargs={"type_": "JSON"})
...
class ComplianceWorkflow(SQLModel, table=True):
    """Model for compliance workflows and automation"""
    ...
    name: str
    description: str
    workflow_type: str  # assessment, remediation, approval, review, notification, escalation
    ...
    # Execution History
    execution_history: List[Dict[str, Any]] = Field(default_factory=list, sa_column_kwargs={"type_": "JSON"})
