...
class ComplianceRequirement(SQLModel, table=True):
    ...
    evidence_files: List[str] = Field(default_factory=list, sa_column_kwargs={"type_": "JSON"})
    documentation_links: List[str] = Field(default_factory=list, sa_column_kwargs={"type_": "JSON"})
...
class ComplianceAssessment(SQLModel, table=True):
    ...
    # Findings
    findings: List[Dict[str, Any]] = Field(default_factory=list, sa_column_kwargs={"type_": "JSON"})
