...
class TeamCollaborationHub(SQLModel, table=True):
    ...
    # Relationships
    members: List["TeamMember"] = Relationship(back_populates="hub")
    reviews: List["RuleReview"] = Relationship(back_populates="collaboration_hub")
    discussions: List["Discussion"] = Relationship(back_populates="hub")
...
class TeamMember(SQLModel, table=True):
    """
    Team member management with advanced role-based permissions.
    ...
    reviews: List["RuleReview"] = Relationship(back_populates="reviewer")
...
class RuleReview(SQLModel, table=True):
    ...
    reviewer: Optional[TeamMember] = Relationship(back_populates="reviews")
...
class Comment(SQLModel, table=True):
    ...
    # User tracking
    author_id: str = Field(foreign_key="team_members.member_id", index=True)
    resolver: Optional[str] = Field(max_length=255)
    last_editor: Optional[str] = Field(max_length=255)
    
    # Relationships
    author: Optional[TeamMember] = Relationship(back_populates="comments")
...
class Discussion(SQLModel, table=True):
    """
    Advanced discussion threads with decision tracking.
