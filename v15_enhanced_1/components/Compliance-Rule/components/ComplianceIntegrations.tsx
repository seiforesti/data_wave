...
<<<<<<< HEAD
        ...
        if (filters.integration_type) {
          params.integration_type = filters.integration_type
        }
        if (filters.provider) {
          params.provider = filters.provider
        }

        const response = await ComplianceAPIs.Integration.getIntegrations(params)
        
        // Filter by data source if specified
        const filteredIntegrations = dataSourceId 
          ? response.filter(integration => integration.data_source_id === dataSourceId)
          : response
        
        setIntegrations(filteredIntegrations)
        
        // Load integration status for each integration
        const integrationsWithStatus = await Promise.all(
          filteredIntegrations.map(async (integration) => {
            try {
              const status = await ComplianceAPIs.Integration.getIntegrationStatus(integration.id)
              return {
                ...integration,
                current_status: status.status,
                last_sync_status: status.last_synced_at,
                sync_statistics: status.sync_statistics,
                error_message: status.error_message
              }
            } catch (error) {
              console.error(`Failed to get status for integration ${integration.id}:`, error)
              return integration
            }
          })
        )
        ...
        console.error('Failed to load compliance integrations:', error)
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      if (integrations.length > 0) {
        try {
          // Check for integration status updates
            ...
            prevIntegrations.map(integration => {
              ...
              if (statusUpdate) {
                return {
                  ...
                  last_synced_at: statusUpdate.last_synced_at,
                  sync_statistics: statusUpdate.sync_statistics,
